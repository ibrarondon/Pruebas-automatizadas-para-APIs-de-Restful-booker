...
def get_booking_by_ID(ID): #Returns a specific booking based upon the booking id provided
    ...
    return(response)
def get_booking_by_name(name): #Returns bookings with a specific firstname
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"firstname":name})
    return(response)
def get_booking_by_lastname(lastname): #Returns bookings with a specific lastname
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"lastname":lastname})
    return(response)
def get_booking_by_checkin(checkin): #Returns bookings that have a checkin date greater than or equal to the set checkin date. Format must be CCYY-MM-DD
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"checkin":checkin})
    return(response)
def get_booking_by_checkout(checkout): #Returns bookings that have a checkout date greater than or equal to the set checkout date. Format must be CCYY-MM-DD
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"checkout":checkout})
    return(response)
def get_booking_by_lastname_and_checkin(lastname,checkin): #Returns bookings with specific lastname and checkin
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"lastname":lastname, "checkin":checkin})
    return(response)
def get_booking_by_name_and_lastname(name,lastname): #Returns bookings with specific name and lastname
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"firstname":name, "lastname":lastname})
    return(response)
def get_booking_by_checkin_and_checkout(checkin,checkout): #Returns bookings with specific checkin and checkout
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"checkin":checkin, "checkout":checkout})
    return(response)
def get_booking_by_lastname_and_checkout(lastname,checkout): #Returns bookings with specific lastname and checkout
    response = requests.get(configuration.URL_SERVICE + configuration.BOOKINGS, params={"lastname":lastname, "checkout":checkout})
    return(response)
def get_booking_by_name_lastname_and_checkout(name,lastname,checkout): #Returns bookings with specific firstname, lastname and checkout
    ...
    return(response)
def get_booking_by_name_lastname_and_checkin(name,lastname,checkin): #Returns bookings with specific firstname, lastname and checkin
    ...
    return(response)
def get_booking_by_name_checkin_and_checkout(name,checkin,checkout): #Returns bookings with specific firstname, checkin and checkout
...
def update_booking_totalprice(id, totalprice):   #Updates the total price for the booking
    ...
    response = requests.put(configuration.URL_SERVICE + configuration.BOOKING_BY_ID + str(id), headers=data.booking_header, json = booking_body)
    return response
def update_booking_depositpaid(id, depositpaid):   #Updates whether the deposit has been paid or not (T/F)
    authToken = create_token().json()["token"]
    data.booking_header ={"Content-Type": "application/json", "Accept": "application/json", "Cookie": "token=" + authToken}
    booking_body = get_booking_by_ID(id).json()
    ...
    response = requests.put(configuration.URL_SERVICE + configuration.BOOKING_BY_ID + str(id), headers=data.booking_header, json = booking_body)
    return response
def update_booking_checkin(id, checkin):   #Updates the checkin date
    authToken = create_token().json()["token"]
    data.booking_header ={"Content-Type": "application/json", "Accept": "application/json", "Cookie": "token=" + authToken}
    booking_body = get_booking_by_ID(id).json()
    ...
    response = requests.put(configuration.URL_SERVICE + configuration.BOOKING_BY_ID + str(id), headers=data.booking_header, json = booking_body)
    return response
def update_booking_checkout(id, checkout):   #Updates the checkout date
    authToken = create_token().json()["token"]
    data.booking_header ={"Content-Type": "application/json", "Accept": "application/json", "Cookie": "token=" + authToken}
    booking_body = get_booking_by_ID(id).json()
